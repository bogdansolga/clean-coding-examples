...
/**
 * Class used to produce a {@link PatientCreatedAugeroDomainEventV1}
 */
@NoArgsConstructor(access = AccessLevel.PRIVATE)
public abstract class AugeroPatientCreatedEventProducer {
    ...
    public static PatientCreatedAugeroDomainEventV1 produce(Patient patient) {

        AugeroEventDataV1 eventData = new AugeroEventDataV1(patient.getId(), creteEndpointUri(patient.getId()),
                patient.getId(), null);

        return new PatientCreatedAugeroDomainEventV1(eventData, PATIENT_ADMIN_SOURCE);
    }
